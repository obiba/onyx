 ...
 * 
 */
public class RadioGroupView extends DataViewBase {
  ...
  public RadioGroupView(String id, ICellPopulator[] populators, IDataProvider dataProvider) {
    ...
    this.radioGroups = new RadioGroup[dataProvider.size()];
  }
   ...
   * @return
   */
  protected RadioGroup newRadioGroup(final String id, int index) {
  ...
  public RadioGroup[] getRadioGroups() {
    return radioGroups;
  }

  public RadioGroup getRadioGroup(int index) {
    return radioGroups[index];
  }
  ...
  protected final void populateItem(Item item) {
    RadioGroup group = newRadioGroup(RADIO_GROUP_ID, item.getIndex());
    radioGroups[item.getIndex()] = group;
    item.add(group);
