...
public class QuestionnaireStageVariableSourceFactory implements VariableValueSourceFactory {
  ...
  protected void buildQuestionnaireRun() {
    ...
    factory.setProperties(ImmutableSet.of("questionnaireVersion", "locale", "timeStart", "timeEnd", "userName"));
    factory.setPropertyNameToVariableName(ImmutableMap.of("questionnaireVersion", "version", "userName", "user"));
  ...
  private class QuestionVariableBuilder {
    ...
    private void buildCategoricalVariable() {
      ...
      if(question.isBoilerPlate() == false) {
        vvsSetBuilder.add(new BeanPropertyVariableValueSource(questionVariable.build(), CategoryAnswer.class, "categoryName"));
      } else {
        vvsSetBuilder.add(new BeanPropertyVariableValueSource(questionVariable.build(), QuestionAnswer.class, "active"));
      }
  ...
  private class QuestionBuilderVisitor extends LocalizableBuilderVisitor {
    ...
    protected void visitAttributes(AttributeAwareBuilder<?> builder1) {
      ...
      List<Attribute> attributes = Attributes.overrideAttributes(question.hasParentQuestion() ? question.getParentQuestion().getAttributes() : null, question.getAttributes());
  ...
  private class QuestionCategoryBuilderVisitor extends LocalizableBuilderVisitor {
    ...
    protected void visitAttributes(AttributeAwareBuilder<?> builder1) {
      ...
      if(this.questionCategory.getQuestion().isMultiple() == false) {
        ...
        builder1.addAttribute(Attribute.Builder.newAttribute("exclusiveChoiceCategoryVariable").withValue(BooleanType.get().valueOf(true)).build());
      }

      // a category variable is implicitly always conditioned by it's parent question
      OnyxAttributeHelper.addConditionAttribute(builder1, new QuestionnaireDataSource(bundle.getQuestionnaire().getName(), questionCategory.getQuestion().getName()));
  ...
  private class OpenAnswerVisitor extends LocalizableBuilderVisitor {
    ...
    public void visit(Builder builder1) {
      ...
      Question question = questionCategory.getQuestion();
      Category category = questionCategory.getCategory();
      List<Attribute> attributes = Attributes.overrideAttributes(question.hasParentQuestion() ? question.getParentQuestion().getAttributes() : null, question.getAttributes(), category.getAttributes(), oad.getAttributes());
