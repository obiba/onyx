...
public class InstrumentTypeFactoryBeanTest {
  ...
  private Map<String, InstrumentType> instrumentTypeMap;
  ...
  public void setup() throws Exception {
    ...
    instrumentTypes = (List<InstrumentType>) instrumentTypeFactory.getObject();

    // Create an instrument type map for easier access to specific instruments.
    instrumentTypeMap = new HashMap<String, InstrumentType>();
    for(InstrumentType instrumentType : instrumentTypes) {
      instrumentTypeMap.put(instrumentType.getName(), instrumentType);
    }
  ...
  public void testInstruments() {
    ...
    // Testing instruments
    Assert.assertEquals(1, instrumentTypeMap.get("Impedence310").getInstruments().size());
    Assert.assertEquals(2, instrumentTypeMap.get("BloodPressure").getInstruments().size());
    Assert.assertEquals(1, instrumentTypeMap.get("StandingHeight").getInstruments().size());
    Assert.assertEquals(1, instrumentTypeMap.get("ECG").getInstruments().size());

    // Testing one instrument attributes
    InstrumentType spirometryInstrumentType = instrumentTypeMap.get("Spirometry");
